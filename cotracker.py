...
def main(args):
    ...
    video = (
        torch.tensor(mediapy.read_video(args.video_path))
        .permute(0, 3, 1, 2)
        .unsqueeze(0)
        .to(device)
        .float()
    ...
    if args.online_mode:
        ...
        for ind in tqdm(
            range(0, video.shape[1] - cotracker_model.step, cotracker_model.step)
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--online_mode", action="store_true", help="Use online mode for tracking."
    )
    parser.add_argument(
        "--grid_size",
        type=int,
    ...
    parser.add_argument(
        "--video_output", action="store_true", help="Save video output visualization."
    )
    parser.add_argument(
